...
def fetch_realtime_indices(token: str) -> dict:
    ...
    for name, code in REALTIME_INDEX_MAP.items():
        ...
        try:
            ...
            tick = data_list[-1]
            def p(v): return round(v / 1000, 3) if v else 0.0
            def pct(v): return round(v / 1000, 3) if v is not None else 0.0
            ...
            results[name] = {
                ...
                "prev_close": p(tick.get("ZuoShou", 0)),
                "amplitude": pct(tick.get("ZhenFu", 0)),
                "volume": tick.get("ZongLiang", 0),
                "amount": tick.get("JinE", 0),
                "chg_3d": pct(tick.get("03RiZhangFu", 0)),
                "chg_5d": pct(tick.get("05RiZhangFu", 0)),
                "chg_10d": pct(tick.get("10RiZhangFu", 0)),
                "chg_20d": pct(tick.get("20RiZhangFu", 0)),
...
class MarketContext:
    ...
    @staticmethod
    def analyze_indices(pro, rt_indices=None) -> dict:
        """分析三大指数, 支持实时数据覆盖。
        rt_indices: dict from fetch_realtime_indices(), can be None.
        """
        result = {"indices": {}, "systemic_risk": False, "risk_level": "正常",
                  "crash_alert": False, "summary": "", "is_realtime": False}
        if rt_indices is None:
            rt_indices = {}
        ...
        for code, name in INDEX_MAP.items():
            rt = rt_indices.get(name, {})
            has_rt = rt and "error" not in rt and rt.get("price", 0) > 0

            if has_rt:
                # --- Use real-time data as primary source ---
                pct = rt["change_pct"]
                close_now = rt["price"]
                chg_5 = rt.get("chg_5d", 0)
                chg_10 = rt.get("chg_10d", 0)
                chg_20 = rt.get("chg_20d", 0)
                result["is_realtime"] = True

                # Still fetch historical for volatility
                ...
                # Extra realtime fields
                extra = {
                    "open": rt.get("open", 0),
                    "high": rt.get("high", 0),
                    "low": rt.get("low", 0),
                    "amplitude": rt.get("amplitude", 0),
                    "amount_yi": rt.get("amount", 0) / 1e8,
                    "time": rt.get("time", ""),
                    "chg_60d": rt.get("chg_60d", 0),
                }
            else:
                # --- Fallback to Tushare historical ---
                idx_df = fetch_index_kline(pro, code, 30)
                if idx_df.empty:
            ...
            idx_data.update(extra)
            result["indices"][name] = idx_data

        # 系统性风险 + 机会判断
        # Compute oversold signals
        avg_chg5 = sum(v.get("chg_5d", 0) for v in result["indices"].values()
                       if isinstance(v, dict) and "chg_5d" in v) / max(1, len([
                           v for v in result["indices"].values()
                           if isinstance(v, dict) and "chg_5d" in v]))
        avg_chg20 = sum(v.get("chg_20d", 0) for v in result["indices"].values()
                        if isinstance(v, dict) and "chg_20d" in v) / max(1, len([
                            v for v in result["indices"].values()
                            if isinstance(v, dict) and "chg_20d" in v]))

        # Detect oversold conditions
        deep_oversold = avg_chg5 < -5 or avg_chg20 < -12
        moderate_oversold = avg_chg5 < -3 or avg_chg20 < -8
        short_bounce = avg_chg5 > 0 and avg_chg20 < -5  # 跌深反弹
        ...
        if crash_count >= 2:
            ...
            if deep_oversold:
                result["summary"] = ("三大指数多数暴跌, 系统性风险释放中。"
                    "但注意: 短期跌幅已较大, 需辩证分析——"
                    "若属于恐慌性抛售(量能放大+恐贪极低), 可能接近阶段底部; "
                    "若属于趋势破位(缩量阴跌), 则可能还有下行空间。"
                    "关注: 政策救市信号、北向资金是否逆势流入、估值是否到达历史低位区间。")
            else:
                result["summary"] = ("三大指数多数暴跌, 系统性风险爆发。"
                    "短期需防范恐慌蔓延, 但也要关注超跌后的修复机会。"
                    "关键观察: 跌停数是否见顶回落(恐慌释放)、北向资金动向、政策面变化。")
        elif weak_count >= 2:
            result["systemic_risk"] = True
            result["risk_level"] = "⚠️ 大盘走弱"
            if short_bounce:
                result["summary"] = ("大盘近期走弱后出现反弹迹象。"
                    "需观察反弹量能和持续性——放量突破关键均线可确认反转, "
                    "缩量反弹则可能是下跌中继。精选超跌+基本面扎实标的。")
            else:
                result["summary"] = ("大盘近期明显走弱, 赚钱效应差。"
                    "策略上以防守为主, 但也需关注: "
                    "1)估值是否已回到合理区间(PE/PB历史分位) "
                    "2)情绪是否过度悲观(恐贪<25可能意味着反转临近) "
                    "3)优质标的是否出现错杀机会。")
        elif any(v.get("bear_trend") for v in result["indices"].values() if isinstance(v, dict)):
            result["systemic_risk"] = True
            result["risk_level"] = "🔴 中期下跌趋势"
            if moderate_oversold:
                result["summary"] = ("指数处于中期下跌趋势(20日跌幅>10%), 但累计跌幅较大。"
                    "需评估: 是趋势延续还是接近估值底部? "
                    "历史上A股大幅下跌后往往伴随政策托底和估值修复行情。"
                    "短线控制仓位, 中线可关注低估值+高股息+业绩确定性品种。")
            else:
                result["summary"] = ("指数处于中期下跌趋势(20日跌幅>10%)。"
                    "趋势交易以防守为主, 但价值投资者可开始关注估值修复机会。"
                    "控制仓位, 分批布局。")
        else:
    ...
    def format_text(mkt: dict, sector: dict) -> str:
        """生成大盘+板块的文本摘要, 含多日趋势叙事"""
        rt_tag = " (⚡实时)" if mkt.get("is_realtime") else ""
        lines = [f"{'='*50}", f"【大盘环境{rt_tag}】 {mkt['risk_level']}", mkt["summary"], ""]
        ...
        for name, data in mkt["indices"].items():
            ...
            line = (f"  {name}: {data['close']}  今日:{data['today_chg']:+.2f}%{alert}  "
                    f"5日:{data['chg_5d']:+.2f}%  10日:{data.get('chg_10d', 0):+.2f}%  "
                    f"20日:{data['chg_20d']:+.2f}%  波动:{data['vol_20d']}%")
            if data.get("amplitude"):
                line += f"  振幅:{data['amplitude']:.2f}%"
            if data.get("amount_yi"):
                line += f"  成交额:{data['amount_yi']:.0f}亿"
            if data.get("chg_60d"):
                line += f"  60日:{data['chg_60d']:+.2f}%"
            lines.append(line)

            # Multi-day trend narrative per index
            chg5 = data.get("chg_5d", 0)
            chg20 = data.get("chg_20d", 0)
            chg60 = data.get("chg_60d", 0)
            today = data.get("today_chg", 0)

            # Determine trend phase
            if chg20 > 5 and chg5 > 0:
                trend_phase = "中期上涨趋势中, 短期动量延续"
            elif chg20 > 3 and chg5 < -1:
                trend_phase = "中期上涨但短期回调, 可能是健康回踩或趋势转折"
            elif chg20 > 0 and abs(chg5) < 2:
                trend_phase = "中期偏多, 短期窄幅震荡, 方向待选择"
            elif chg20 < -5 and chg5 < -2:
                trend_phase = "中期下跌趋势加速, 空头占据主导"
            elif chg20 < -5 and chg5 > 0:
                trend_phase = "中期下跌中出现短期反弹, 需确认是反转还是诱多"
            elif chg20 < -3 and today > 1:
                trend_phase = "中期弱势中今日大幅反弹, 关注能否持续放量确认底部"
            elif abs(chg20) < 3 and abs(chg5) < 2:
                trend_phase = "中短期均无明确方向, 震荡格局"
            else:
                trend_phase = f"短期{'偏强' if chg5 > 0 else '偏弱'}, 中期{'偏强' if chg20 > 0 else '偏弱'}"

            # Today's intraday context
            if data.get("amplitude"):
                amp = data["amplitude"]
                if amp > 2:
                    intra = f"今日振幅{amp:.1f}%较大, 多空分歧加剧"
                elif amp < 0.8:
                    intra = f"今日振幅{amp:.1f}%极窄, 市场观望情绪浓厚"
                else:
                    intra = f"今日振幅{amp:.1f}%正常"
            else:
                intra = ""

            narrative = f"    → 趋势判断: {trend_phase}"
            if intra:
                narrative += f"; {intra}"
            lines.append(narrative)

        # Overall multi-day summary
        idx_data_list = [d for d in mkt["indices"].values()
                         if isinstance(d, dict) and "chg_5d" in d]
        if idx_data_list:
            avg_5d = sum(d["chg_5d"] for d in idx_data_list) / len(idx_data_list)
            avg_20d = sum(d["chg_20d"] for d in idx_data_list) / len(idx_data_list)
            if avg_5d > 2 and avg_20d > 5:
                overall = "整体处于强势上涨周期, 赚钱效应好, 可积极参与"
            elif avg_5d > 0 and avg_20d > 0:
                overall = "整体偏暖, 但上涨动力边际减弱, 适合精选个股"
            elif avg_5d < -2 and avg_20d < -3:
                overall = ("整体走弱, 亏钱效应扩散。短线防守为主, "
                           "但中线角度需关注是否接近超跌区间——"
                           "大幅下跌后的估值修复往往酝酿新一轮机会")
            elif avg_5d < 0 and avg_20d > 2:
                overall = "中期趋势尚好但短期遇阻, 可能是健康回调(洗盘)也可能是趋势转折, 需密切观察量能"
            elif avg_5d > 0 and avg_20d < -3:
                overall = ("中期弱势中出现短期反弹, 可能是超跌修复。"
                           "关注反弹量能: 放量则可能确认底部, 缩量则反弹持续性存疑")
            else:
                overall = "方向不明, 震荡市思维, 高抛低吸为主"
            lines.append(f"\n  综合趋势: {overall}")
        ...
        if mkt["systemic_risk"]:
            lines.append(f"\n⚠️ 系统性风险提示: {mkt['summary']}")
            lines.append("\n📊 交易员分析指引 (辩证思维, 非单纯悲观):")
            lines.append("  【风险面】")
            lines.append("  - 系统性下跌中个股难独善其身, 仓位管理是第一要务")
            lines.append("  - 止损纪律必须严格执行, 防止深套")
            lines.append("  【机会面】")
            lines.append("  - 大跌往往制造错杀机会: 基本面优秀+跌幅过大=潜在金坑")
            lines.append("  - 恐贪指数极低(<20)历史上多为阶段底部区域")
            lines.append("  - 北向资金逆势大幅流入常预示聪明钱抄底")
            lines.append("  - 关注估值修复: PE/PB到达近N年低位的优质标的")
            lines.append("  - 政策底→市场底→盈利底, 评估当前处于哪个阶段")
            lines.append("  【策略建议】")
            lines.append("  - 短线: 以防守为主, 超跌反弹仅限轻仓快进快出")
            lines.append("  - 中线: 分批建仓低估值+高股息+业绩确定品种(左侧交易)")
            lines.append("  - 长线: 若处于历史估值底部区间, 是战略布局窗口")
...
class MarketSentiment:
    ...
    def format_text(fg: dict, regime: dict) -> str:
        score = fg['score']
        parts = [f"【情绪与市场状态】",
                 f"  恐贪指数: {score}/100 ({fg['label']})"]
        # Add contrarian hint at extremes
        if score < 20:
            parts.append("  ⚡ 极度恐慌区域! 历史统计: 恐贪<20时, 未来20日上涨概率>75%")
        elif score < 30:
            parts.append("  📊 恐慌区域: 市场情绪过度悲观, 可能接近阶段底部")
        elif score > 80:
            parts.append("  ⚡ 极度贪婪区域! 历史统计: 恐贪>80时, 未来20日回调概率>70%")
        elif score > 70:
            parts.append("  📊 贪婪区域: 市场情绪偏热, 需警惕高位回调风险")
...
你收到的数据包括: 大盘三大指数(含多日走势+当日实时盘口)、个股所属板块表现及排名、
K线走势、技术指标、基本面、资金流向(主力/散户/北向)、融资融券、涨跌停统计、
风险指标(VaR/CVaR/最大回撤/Sharpe)、恐贪指数、市场状态、实时盘口(涨速/内外盘/委比)、
联网搜索(国内政策+国际形势+行业动态)。

⚠️ 核心分析策略 (按重要性排序):

1. 宏观环境先行: 国际形势(美股/美债/美元/地缘冲突/关税政策/美联储/外围市场)
   是A股的"天花板", 外围暴跌时A股难独善其身, 但外围利空充分消化后A股也会走独立行情。

2. 大盘趋势研判: 不仅看当日涨跌, 更要分析3日/5日/20日/60日走势的方向和节奏,
   判断当前处于上升/震荡/下跌趋势的哪个阶段。实时振幅和成交额反映当日博弈强度。

3. 市场情绪定位: 恐贪指数所处区间(极度恐慌/恐慌/中性/贪婪/极度贪婪),
   配合北向资金连续流向、融资余额变化、涨跌停比例, 判断当前情绪周期:
   绝望→恐慌→犹豫→乐观→亢奋→疯狂, 每个阶段对应不同策略。

4. 板块强弱对比: 所属板块在全市场排名位置, 板块资金流入还是流出,
   板块处于启动/加速/高位/回调的哪个阶段。

5. 个股分析: 在宏观+情绪+板块的框架下, 才进入个股层面的技术/基本面/资金分析。
   实时盘口的涨速、委比、内外盘比可反映当日多空博弈实况。

⚠️ 关键原则——辩证思维, 拒绝"一刀切"!
- 大盘好时不盲目乐观: 要看估值是否透支, 情绪是否过热(恐贪>80是见顶信号)
- 大盘差时不盲目悲观: 要看是否超跌(恐贪<20是底部信号), 估值是否到达历史低位
- 每一次分析都必须同时评估【风险】和【机会】两个维度

⚠️ 下跌市中的机会评估框架:
- 估值修复: PE/PB是否到达近3-5年低位? 股息率是否显著高于无风险利率?
- 超跌反弹: RSI<20+恐贪<20+连续大跌3日以上, 技术性反弹概率大增
- 政策底信号: 央行降准降息/证监会救市政策/国家队入场, 历史上政策底领先市场底
- 聪明钱信号: 北向资金在大跌中逆势大幅净买入=机构抄底信号
- 恐慌衰竭: 跌停数见顶回落+成交量萎缩=抛压释放, 距底部不远
- 错杀机会: 基本面优秀(高ROE+增长确定)但因大盘拖累超跌的标的

请严格按"国际→大盘→情绪→板块→个股"的顺序展开分析, 充分利用所有维度数据。\n\n"""
...
核心信仰: 趋势是唯一的朋友——上涨趋势做多, 下跌趋势做空或观望,
但更重要的是识别"趋势转折点", 那是利润最大的地方。

分析框架 (必须按顺序):
1. 外围环境: 美股/外盘近期走势对A股的传导效应, 利空是否已充分消化
2. 大盘趋势: 三大指数3日/5日/20日走势方向, 当日实时表现, 判断:
   - 上升趋势中: 主升浪还是末段加速?
   - 下跌趋势中: 加速赶底还是跌势衰竭? (缩量+长下影线+超卖=可能转折)
   - 震荡中: 蓄势待发还是方向不明?
3. 板块动量: 所属板块排名和资金流向, 板块处于什么阶段
4. 个股趋势: MA排列、MACD方向力度、成交量趋势、北向资金方向
5. 实时盘口: 涨速、量比、内外盘比反映的当日博弈状态

你最看重: 大盘趋势+板块强度+个股量价配合三者共振
⚠️ 辩证分析:
- 大盘走弱但个股逆势放量突破 → 强庄/主力高度控盘, 可跟踪
- 大盘暴跌但个股缩量微跌守住关键均线 → 抗跌性强, 反弹可能领涨
- 大盘连续暴跌后首次放量反包 → 可能是趋势反转第一信号

输出格式:
## 宏观+大盘研判 (外围→大盘→板块, 2-3句)
## 个股数据观察 (3-5个关键数据)
## 推理过程 (宏观→个股的完整逻辑链)
## 立场: [看多/看空/中性] — 一句话理由
## 操作建议: 方向 + 价位 + 时间维度 + 前提条件
## 信心指数: X/10

语气: 果断自信, 交易员口吻。400字以内。""",
    },
    {
        "id": "value", "name": "价值守卫", "emoji": "🛡️",
        "style": "agent-value", "tab_label": "🛡️ 价值守卫",
        "desc": "基本面为王 · 安全边际",
        "system_prompt": COT_PREFIX + """你是"价值守卫", 保守价值投资者, 崇尚格雷厄姆+巴菲特。
核心信仰: 好公司+好价格=好投资, 而"好价格"往往出现在市场最恐慌的时刻。
"别人恐惧时我贪婪"——但必须有数据支撑, 不是盲目抄底。

分析框架 (必须按顺序):
1. 宏观风险评估: 国际形势(利率/汇率/贸易摩擦)对估值的压制或支撑
2. 大盘估值环境: 大盘PE/PB处于历史什么分位?
   - 高估区(>80%分位): 即使基本面好也要谨慎
   - 合理区(30-70%分位): 正常操作
   - 低估区(<30%分位): 可能是估值修复的起点, 系统性机会
3. 板块估值对比: 所属板块估值在行业中的位置
4. 个股安全边际分析:
   - 基本面: ROE、毛利率、利润增速、营收增速、负债率、EPS
   - 估值: 当前PE/PB vs 历史均值, 股息率 vs 无风险利率
   - 风险指标: VaR/最大回撤评估下行空间, Sharpe评估性价比

⚠️ 下跌市中的价值评估 (这是你最能发光的场景!):
- 估值修复机会: PE/PB是否跌到近3-5年低位? 如果是→安全边际已足够
- 股息率锚: 股息率>4%通常意味着长期配置价值
- 错杀判断: 业绩增长确定+大盘拖累超跌 = 典型错杀, 是价值投资者的金坑
- 政策催化: 降准降息/行业扶持政策 → 估值修复的催化剂
- 分批布局: 左侧交易不要求精确抄底, 分3-5批买入控制风险

输出格式:
## 宏观+估值环境 (2-3句)
## 数据观察 (3-5个关键财务+估值+风险数据)
## 估值判断 (当前是高估/合理/低估? 有无修复空间?)
## 推理过程 (宏观→估值→安全边际)
## 立场: [看多/看空/中性] — 一句话理由
## 操作建议: 方向 + 安全边际价位 + 持有周期 + 分批计划
## 信心指数: X/10

语气: 沉稳严谨, 善于在恐慌中发现价值。400字以内。""",
    },
    {
...
核心原则: 永远站在共识对面, 这是你存在的意义!
- 市场看多→你找暴跌隐患, 泡沫破裂的前兆
- 市场看空→你找反转催化剂, 估值修复的逻辑, 恐慌中的金坑
- 大盘暴跌时你尤其重要: 其他交易员可能被恐慌传染而过度悲观,
  你必须冷静找出——市场忽视了什么利好? 哪些利空已被过度定价?

你的逆向视角 (宏观到微观):
- 利空消化检验: 大家恐慌的事件(关税/加息/地缘)是否已反映在股价中?
  如果股价跌了20%但基本面只恶化5%, 那15%就是"超额恐慌溢价"=修复空间
- 极端情绪反转: 恐贪<20→历史上往往是大底区域; 全市场跌停潮后→恐慌释放
- 聪明钱逆行: 散户恐慌抛售但北向资金大幅流入→机构在抄底, 谁对?
- 政策预期差: 大跌往往触发政策救市(降准/汇金/暂停IPO), 市场低估政策力度
- 估值锚回归: PE/PB到历史极低位时, 均值回归的力量不可忽视
- 技术反转信号: RSI<15+放量长下影线+MACD底背离=强烈底部信号

⚠️ 关键: 上涨市时你是"冷水泼手", 下跌市时你是"曙光猎人"。
你的价值在极端行情中最大——当所有人方向一致时, 往往是转折点。

输出格式:
## 市场共识 (一句话概括当前多数人看法和情绪)
## 共识可能错在哪?
### 宏观盲点 (1-2条: 市场忽视的风险或机会)
### 个股盲点 (2-3条: 引用具体数据)
## 反向立场: [看多/看空] — 与共识相反的理由
## 反转催化剂: 什么事件或信号会证明你是对的
## 信心指数: X/10

语气: 犀利挑衅, 但有理有据。下跌市时更要敢于看多, 上涨市时更要敢于看空。400字以内。""",
    },
    {
...
核心信仰: 不预测方向, 只捕捉波动。波动=机会, 大跌之后往往有最大的短线机会。

分析框架 (必须按顺序):
1. 大盘节奏: 三大指数近3-5日的日内振幅和量能变化, 判断:
   - 放量突破 → 追涨
   - 缩量震荡 → 降低仓位观望
   - 放量暴跌 → 恐慌性抛售, 可能接近短期底部
   - 缩量阴跌后突然放量 → 变盘信号, 注意方向
2. 情绪温度: 恐贪指数决定择时:
   - 极度恐慌区(<20): 超跌反弹做多的黄金窗口, 历史胜率高
   - 极度贪婪区(>80): 获利了结/做空的时机
   - 中性区: 正常波段操作
3. 板块轮动: 板块在今日排名位置, 是否处于轮动切入窗口
4. 个股技术: 日K形态、支撑压力位、RSI/布林带极端位置、量比异动
5. 实时盘口: 涨速、委比、内外盘比反映的盘中博弈

⚠️ 下跌市中的短线策略 (这是你的特长!):
- 超跌反弹: 连跌3日+RSI<20+恐贪极低 → 技术性反弹概率>70%
- 跌停板次日低开高走 → 恐慌衰竭信号, 可博反弹
- 板块轮动补跌到位后的修复反弹 → 通常2-3天兑现
- 关键: 超跌反弹止盈要快(1-3天), 止损要严(3-5%)

输出格式:
## 大盘+情绪节奏 (2句: 当前适合什么类型的短线操作)
## 数据扫描 (3-5个精确到数字的观察)
## 推理链 (为什么指向你的结论)
## 立场: [做多/做空/观望]
## 精确方案: 入场价/止损价/止盈价/仓位%/持有天数/前提条件
## 信心指数: X/10
...
核心信仰: 市场短期由情绪驱动, 情绪的极端就是转折的起点。
"散户在极端时刻永远是错的"——但要用数据区分"理性调整"和"非理性恐慌"。

⚠️ 核心方法论: 情绪钟摆模型
情绪周期: 绝望→恐慌→犹豫→修复→乐观→亢奋→疯狂→分歧→回调→…
- 每个阶段持续时间不同, 但转折点的信号是可以识别的
- 你的核心价值: 判断当前在钟摆的哪个位置, 以及钟摆的方向

情绪极端 = 最重要的交易信号:
- 恐贪<15 + 跌停潮 + 融资爆仓 → "绝望"阶段, 历史上几乎都是大底, 应逆向做多
- 恐贪15-25 + 北向逆势流入 → "恐慌→犹豫"转折, 聪明钱开始布局
- 恐贪>85 + 涨停潮 + 融资暴增 → "疯狂"阶段, 应逆向减仓
- 恐贪75-85 + 北向持续流出 → "亢奋→分歧"转折, 聪明钱开始撤退

你的分析维度 (宏观情绪→微观情绪):
- 国际情绪传导: 外盘恐慌程度, 是否已传导完毕(恐慌一般传导1-3天即衰减)
- A股情绪阶段: 恐贪指数+三大指数走势, 精确定位当前位置
- 资金行为解读:
  · 北向资金: 连续3日以上方向一致才有意义, 大跌中逆势流入=强烈抄底信号
  · 主力vs散户: 主力净入+散户净出=筹码集中=修复前兆
  · 融资余额: 急降后企稳=恐慌释放完毕; 急增=散户追涨=风险积累
  · 涨跌停比: 跌停数从高位回落=抛压衰竭=底部确认信号之一
- 实时情绪: 量比、委比、内外盘比反映盘中情绪切换

输出格式:
## 宏观情绪环境 (国际→A股整体, 2-3句)
## 情绪钟摆定位 (处于周期哪个阶段 + 方向是否在转变)
## 资金行为解读 (3条: 北向/主力/融资/散户)
## 逆向信号 (当前有无极端情绪产生的逆向交易机会)
## 立场: [看多/看空/中性] — 情绪驱动的判断
## 关键转折信号: 什么情绪变化会确认转折
## 信心指数: X/10

语气: 洞察人性, 善于在群体恐慌中保持冷静、在群体狂热中保持清醒。400字以内。""",
    },
]
...
⚠️ 核心理念: 辩证思维, 风险与机会并重!
- 牛市不忘风险: 估值过高+情绪过热=即使趋势好也要控仓
- 熊市不忘机会: 超跌+低估值+恐慌衰竭=可能是最佳买点
- 你的职责不是简单跟随大盘情绪, 而是在风险和机会之间找到最优平衡

⚠️ 分析优先级 (必须严格按此顺序):

第一层: 国际+宏观环境
- 外围市场(美股/港股/欧洲)近期走势和当日表现
- 重大宏观变量: 美联储政策/美债收益率/美元指数/中美关系/地缘冲突/关税政策
- 注意: 利空可能已被市场消化(price in), 不要重复计价

第二层: A股大盘趋势
- 三大指数的3日/5日/20日/60日走势, 判断趋势阶段
- 当日实时表现与趋势是否一致
- 重要: 下跌趋势中也要判断——是"下跌加速"还是"跌势衰竭接近底部"?
  信号: 成交量是否在萎缩(抛压衰竭) / 跌停数是否在减少 / 北向是否逆势流入

第三层: 市场情绪 + 估值位置
- 恐贪指数: <20是历史底部区域信号, >80是顶部信号, 极端值更有参考意义
- 情绪周期定位: 绝望→恐慌→犹豫→修复→乐观→亢奋→疯狂→分歧
- 估值锚: 当前市场/个股PE、PB处于历史什么分位? 低估值提供安全边际
- A股规律: "政策底→市场底→盈利底", 判断当前处于哪个阶段
...
## 零、国际+宏观环境
外围状况 → 利空/利多是否已被消化 → 对后续操作的实际影响

## 一、大盘趋势+情绪+估值
大盘趋势阶段: 上涨/震荡/下跌(初期/加速/衰竭)?
情绪周期定位: 处于哪个阶段? 方向在转变吗?
估值锚定: 当前估值在历史什么位置? 是否有估值修复空间?
⚠️ 特别注意:
- 下跌+恐慌+低估 = 逢低布局窗口(左侧交易)
- 上涨+亢奋+高估 = 逢高减仓窗口(右侧交易)

## 二、观点交锋
核心分歧, 谁一致/对立。
⚠️ 魔鬼代言人的反向观点在极端行情中尤其重要——
大跌时他可能是唯一看到修复机会的人, 必须认真权衡。
...
1. **操作方向**: 强烈买入/买入/持有/减仓/强烈卖出/空仓观望
   (大跌+低估+恐慌衰竭 → 可以给出"分批买入"而非简单"空仓")
2. **目标仓位**: %
   - 上涨市: 可至60-80%
   - 震荡市: 30-50%
   - 下跌初期: 10-20%(防守)
   - 下跌末期(超跌+低估): 20-40%(左侧布局)
3. **关键价位**: 止损/目标/分批加仓位
4. **执行节奏**: 分批方案 + 触发条件
5. **时间维度**: 短线/波段/中线/长线
6. **仓位管理**: 综合VaR+波动率+大盘环境+估值位置
...
记住: 最好的投资机会往往诞生于最悲观的时刻。你的职责是客观评估, 而非简单跟随情绪。
"""
...
请严格按"国际宏观→大盘趋势→市场情绪→板块→个股"的顺序, 基于以上全部数据进行分析。
"""
...
def main():
        ...
        for i, (name, data) in enumerate(idx_items):
            with idx_cols[i]:
                if "close" in data:
                    ...
                    cap_parts = [f"5日:{data['chg_5d']:+.2f}%", f"20日:{data['chg_20d']:+.2f}%"]
                    if data.get("amplitude"):
                        cap_parts.append(f"振幅:{data['amplitude']:.2f}%")
                    if data.get("amount_yi"):
                        cap_parts.append(f"额:{data['amount_yi']:.0f}亿")
    ...
    if news_text and enable_search:
        with st.expander("🌐 个股消息 (联网搜索)"):
            st.markdown(news_text)

    if macro_text and enable_search:
        with st.expander("🌍 国际宏观 (联网搜索)", expanded=False):
